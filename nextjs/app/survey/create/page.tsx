...
const CreateSurveyPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    createSurveyMutation.mutate({
      ...
      date,
      group,
      questions // Hier werden die Fragen übergeben
    });
  };
        ...
        <div className="w-full max-w-lg bg-[var(--background-primary)] rounded-xl shadow-lg p-8 transform transition-all duration-300 hover:shadow-xl">
          ...
          {createSurveyMutation.isPending ? (
              ...
              <form onSubmit={handleSubmit} className="max-w-none p-0 shadow-none">
                ...
                {/* Neues Feld für Fragen */}
                <div className="mb-6">
                  <label htmlFor="newQuestion" className="block text-sm font-medium text-[var(--text-primary)] mb-2">
                    Frage hinzufügen
                  </label>
                  <div className="flex gap-2">
                    <input
                        ...
                        className="block w-full rounded-lg border border-[var(--background-tertiary)] bg-[var(--background-secondary)] text-[var(--text-primary)] py-2 px-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-[var(--primary-color)] focus:border-transparent transition-all duration-200 h-full"
                    />
                    <button
                        type="button"
                        onClick={handleAddQuestion}
                        className="px-4 py-2 bg-gradient-to-r from-[var(--accent-color-1)] to-[var(--secondary-color)] text-white font-medium rounded-lg shadow-md hover:shadow-lg transition-all duration-200 w-1/5 h-full mt-0"
                    >
                      +
                    </button>
                  </div>
                  {questions.length > 0 && (
                      <ul className="mt-4 space-y-2">
                        {questions.map((question, index) => (
                            <li key={index} className="flex items-center justify-between bg-[var(--background-tertiary)] rounded p-2">
                              <span className="text-[var(--text-primary)]">{question}</span>
                              <button
                                  type="button"
                                  onClick={() => handleRemoveQuestion(index)}
                                  className="text-[var(--accent-color-1)] w-1/6 bg-transparent border-[var(--accent-color-1)] mt-0"
                              >
                                -
                              </button>
                            </li>
